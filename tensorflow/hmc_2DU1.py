...
class Param:
    ...
    def initializer(self):
        if self.randinit:
            return tf.random_uniform_initializer(-math.pi, math.pi)
        else:
...
def leapfrog(param, x, p):
    ...
    _, x__, pp = tf.while_loop(
        lambda i, xx, pp: i < param.nstep-1,
        body,
        (0, x_, p_))
    xx = x__ + 0.5*dt*pp
    return (xx, pp)
...
if __name__ == '__main__':
    ...
    config = tf.ConfigProto(intra_op_parallelism_threads=param.nth,  # ALCF suggests number of physical cores
    ...
    field = tf.get_variable("field", [param.nd] + param.lat, dtype=tf.float64, initializer=param.initializer()) # mu, x, t
